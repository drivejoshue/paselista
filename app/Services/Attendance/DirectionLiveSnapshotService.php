...
final class DirectionLiveSnapshotService
{
    ...
    public function snapshot(
        ...
        $filters = [
            'campus_id' => ! empty($filters['campus_id'])
                ? (int) $filters['campus_id']
                : null,

            'level_id' => ! empty($filters['level_id'])
                ? (int) $filters['level_id']
                : null,

            'group_id' => ! empty($filters['group_id'])
        ...
        return [
            ...
            'groups' => $groups
                ->map(fn (object $group): array => [
                    ...
                    'early_exit' =>
                        (int) $group->early_exit,
                    'attendance_rate' =>
                        (float) $group->attendance_rate,
                ])
                ->values()
                ->all(),
    ...
    private function summary(
        ...
        return [
            ...
            'early_exit' => $rows
                ->filter(
                    fn ($row) =>
                        (bool) $row->is_early_exit
                )
                ->count(),

            'attendance_rate' => $eligible > 0
                ? round(
                    ($present / $eligible) * 100,
                    1
                )
                : 0.0,
        ];
    ...
    private function recentActivity(
        ...
        return DB::table('access_logs as log')
            ...
            ->leftJoin(
                'access_devices as device',
                'device.id',
                '=',
                'log.access_device_id'
            )
            ->leftJoin(
                'users as operator',
                'operator.id',
            ...
            ->when(
                ...
                function ($query, $id): void {
                    $query->where(
                        fn ($inner) => $inner
                            ->where(
                                'historical_group.campus_id',
                                $id
                            )
                            ->orWhere(
                                'current_group.campus_id',
                                $id
                            )
                    );
                }
            ...
            ->select([
                ...
                DB::raw(
                    'CONCAT('
                    .'COALESCE(guardian.first_name, ""), '
                    .'" ", '
                    .'COALESCE(guardian.last_name, "")'
                    .') as guardian_name'
                ),
